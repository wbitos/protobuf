...
- (BOOL) refillBuffer:(BOOL) mustSucceed {
  ...
  bufferSize = 0;
  if (input != nil) {
    bufferSize = (int)[input read:(uint8_t *)(buffer.mutableBytes) maxLength:buffer.length];
  }
  ...
  if (size <= bufferSize - bufferPos) {
    ...
    while (sizeLeft > 0) {
      ...
      while (pos < chunk.length) {
        int32_t n = 0;
        if (input != nil) {
          n = (int)[input read:(((uint8_t*) chunk.mutableBytes) + pos) maxLength:chunk.length - pos];
        }
        if (n <= 0) {
          @throw [NSException exceptionWithName:@"InvalidProtocolBuffer" reason:@"truncatedMessage" userInfo:nil];
        }
...
- (void) skipRawData:(int32_t) size {
  ...
  if (size <= (bufferSize - bufferPos)) {
    ...
    while (pos < size) {
      ...
      int32_t n = (input == nil) ? -1 : (int32_t)[input read:(uint8_t *)data.mutableBytes maxLength:(size - pos)];
      if (n <= 0) {
        @throw [NSException exceptionWithName:@"InvalidProtocolBuffer" reason:@"truncatedMessage" userInfo:nil];
      }
      pos += n;
